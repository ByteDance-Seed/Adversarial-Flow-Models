...
def download(
    ...
    if dirname is None:
        dirname = "/home/tiger/.cache/vgfm/downloads/"
    if filename is None:
...
def listdir_with_metafile(path: str, overwrite: bool = True, disable: bool = False, separate_meta_storage: bool = False) -> List[str]:
    """
    ...
    if not is_hdfs_path(path) or disable:
        return listdir(path)

    # Define metafile path.
    
    if separate_meta_storage:
        hashpath = hashlib.md5(path.encode("utf-8")).hexdigest()
        metafile = os.path.join("metafile", hashpath+".pkl")
    else:
